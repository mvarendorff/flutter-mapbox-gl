...
class MethodChannelMapboxGl extends MapboxGlPlatform {
  ...
  Future<List<Symbol>> addSymbols(List<SymbolOptions> options,
      [List<Map?>? data]) async {
    final List<dynamic>? symbolIds = await _channel.invokeMethod<List<dynamic>>(
      'symbols#addAll',
      <String, dynamic>{
        'options': options.map((o) => o.toJson()).toList(),
      },
    );
    if (symbolIds == null) return [];
  ...
  Future<LatLngBounds> getVisibleRegion() async {
    try {
      final Map<Object, Object> reply = await (_channel.invokeMethod(
          'map#getVisibleRegion', null) as FutureOr<Map<Object, Object>>);
      LatLng southwest = LatLng(0, 0), northeast = LatLng(0, 0);
